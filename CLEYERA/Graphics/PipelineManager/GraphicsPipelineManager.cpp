...
void GraphicsPipelineManager::Initialize()
{
	...
	CreateLighting3dParticlePSOs(pso);
...
void GraphicsPipelineManager::CreateLighting3dParticlePSOs(SPSO& pso)
{
	ComPtr<ID3D12Device> device = DirectXCommon::GetInstance()->GetDevice();
	Commands commands = DirectXCommon::GetInstance()->GetCommands();
	SShaders shader = ShaderManager::Getinstance()->GetShader();

	pso.LightingParticle3d.none = CreateLightingParticle3dNone(device, commands, shader.LightParticle);
...
SPSOProperty GraphicsPipelineManager::CreateLightingParticle3dNone(ComPtr<ID3D12Device> device, Commands commands, SShaderMode shader)
{
	SPSOProperty ParticlePSO;
	...
	D3D12_DESCRIPTOR_RANGE descriptorRangeForInstancingLight[1] = {};
	descriptorRangeForInstancingLight[0].BaseShaderRegister = 1;
	descriptorRangeForInstancingLight[0].NumDescriptors = 1;
	descriptorRangeForInstancingLight[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	descriptorRangeForInstancingLight[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;


	rootParameters[5].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootParameters[5].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
	rootParameters[5].DescriptorTable.pDescriptorRanges = descriptorRangeForInstancingLight;
	rootParameters[5].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeForInstancingLight);
	...
	inputElementDescs[4].SemanticName = "WORLDPOSITION";
